...
namespace Identity.BusinessLogic.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task SignUpWithFirebaseAsync(AccountForSignUpWithFirebaseDto account)
        {
            var accountEntity = await _unitOfWork.AccountRepository.GetSingleConditionsAsync(
            ...
            await auth.CreateUserWithEmailAndPasswordAsync(
                account.Email, account.Password, account.DisplayName, true);
        }

        public async Task<object> SignInWithFirebaseAsync(AccountForSignInWithFirebaseDto account)
        {
            var firebaseAuthProvider = new FirebaseAuthProvider(
                new FirebaseConfig(_configuration["Authentication:Firebase:WebAPIKey"]));

            var firebaseAuthLink = await firebaseAuthProvider.SignInWithEmailAndPasswordAsync(account.Email, account.Password);
            ...
            var accountEntity = await _unitOfWork.AccountRepository
                .GetSingleConditionsAsync(a => a.Email == account.Email || a.UserName == account.Email);
        ...
        public string GenerateAccessToken(Account account)
        {
            ...
            claimsForToken.Add(new Claim("Sub", account.Id.ToString()));
